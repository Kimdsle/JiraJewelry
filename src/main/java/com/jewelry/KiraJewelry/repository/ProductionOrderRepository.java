...
public interface ProductionOrderRepository extends JpaRepository<ProductionOrder, String> {
        ...
        @Query("SELECT SUM(po.o_Total_Amount) FROM ProductionOrder po WHERE FUNCTION('MONTH', po.date) = FUNCTION('MONTH', CURRENT_DATE) AND FUNCTION('YEAR', po.date) = FUNCTION('YEAR', CURRENT_DATE)")
        Double sumTotalAmountByThisMonth();
